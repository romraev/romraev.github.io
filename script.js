...
const cardCont = document.querySelector(".featured__table");
const popUpCart = document.querySelector(".header__right__details__cart");
const cartList = JSON.parse(localStorage.getItem("cartList")) || [];
const totalPrice = popUpCart.querySelector(
  ".header__right__details__cart__total__price"
);
const enptyMess = document.createElement("p");
enptyMess.classList.add("cart_empty");
enptyMess.textContent = "The cart is empty";

function recalcTotals() {
  totalPrice.textContent = `$${cartList.reduce(
...
if (cartList.length == 0) {
  ...
  cartList.forEach((el) => addToCart(el));
  recalcTotals();
}
...
document.addEventListener("DOMContentLoaded", () => {
  fetch(url)
    ...
    .then((result) => {
      ...
      data.forEach((el) => {
        cardCont.insertAdjacentHTML(
          "beforeend",
          `
        <article class="featured__table__item">
        <div class="featured__img_add">
          <div class="add_to_cart">
            <a href="" class="addtocart_button hover-unfill" id="${el.id}">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="27"
                height="25"
          ...
          <a href="./pages/product.html" class="featured-foto-link">
            <img class="featured__img" src="${el.img}" alt="${el.id}" />
          </a>
        </div>
        <div class="featured__text_price">
          <h3 class="featured__table__item__heading">${el.header}</h3>
          <h3 class="featured__table__item__price">$${el.price}</h3>
        </div>
      </article>
    `
        );
    ...
    .then((data) => {
      const addToCartButtons = document.querySelectorAll("a.addtocart_button");
      addToCartButtons.forEach((button) => {
        button.addEventListener("click", (event) => {
          ...
          if (cartList.findIndex((el) => el.id === id) === -1) {
            cartList.push(...data.filter((obj) => obj.id == id));
            localStorage.setItem("cartList", JSON.stringify(cartList));
            addToCart(cartList[cartList.length - 1], addButton);
          } else {
            const quant = ++cartList[cartList.findIndex((el) => el.id === id)]
              .quantity;
            localStorage.setItem("cartList", JSON.stringify(cartList));
            const quantProd = document.querySelector(`#qi-${id}`);
            quantProd.textContent = quant;
            recalcTotals();
          }
        });
      });
